...
def search():
    try:
        # 1. Sabse pehle parameters fetch honge
        region = request.args.get('server', 'IND').upper()
        search_term = request.args.get('name')
        key = request.args.get("key")

        # 2. API Key check hogi (Ab ye try ke andar hai)
        if key != "Flash":
...
def stats():
    try:
        ...
        # 2. Validate API Key (Step-by-step logic)
        if key != "Flash":
            return jsonify({
                "success": False,
...
def info():
    try:
        ...
        key = request.args.get("key") # Key parameter liya
        ...
        print(f"Unexpected error in info endpoint: {str(e)}")
